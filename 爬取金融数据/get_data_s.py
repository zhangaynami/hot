    ...
    def get_hot_list(getkey,secids,securityCodes,num=20):
        ...
        if tips_response.status_code == 200:
            ...
            for i in range(len(get_data)):
                ...
                tips_content["代码"] = srcSecurityCode
        ...
        all_tips_data["代码"] = all_tips_data["代码"].str[2:]
        result = pd.merge(data, all_tips_data,on='代码', how='left')
        print("获取东方财富热搜成功")
...
def get_funds_data():
    ...
    if response.status_code == 200:
        ...
        data = pd.DataFrame(stock_list)
        get_title = ["f3","f14","f62","f184","f204"]
        # 列名中英文对照字典
        columns_chinese = {
            "f3": "涨跌幅",
            "f14": "行业",
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # 构建完整文件路径
        csv_filename = os.path.join(script_dir, "资金流向板块.csv")
        fin_data.to_csv(csv_filename, index=False, encoding='utf-8-sig')
        print("资金流向板块数据获取完成")
        return fin_data 
    else:
...
def get_youzi(day):
    print("获取游资数据")
    url = f"https://ushq.stock.sohu.com/finance/capital-map-service/capital-data/hot-capital?date={day}&callback=hotmoney&_=1739416548569"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
    }
    response = requests.get(url, headers=headers)
    text = response.text
    json_str = json.loads(re.search(r'^[^(]*\((.*)\);?$', text).group(1))  
    get_data = json_str.get("data")  
    if response.status_code == 200:
        while get_data == None:
            day = get_yesterday_date(day)
            print(day)
            url = f"https://ushq.stock.sohu.com/finance/capital-map-service/capital-data/hot-capital?date={day}&callback=hotmoney&_=1739416548569"
            response = requests.get(url, headers=headers)
            text = response.text
            ...
            new_data = []
            for i in range(len(get_data)):
                stocks = get_data[i]['stockInfo']
                stock_codes = []
                stock_names = [] 
                stock_buy = []
                for stock in stocks:
                    stock_codes.append(stock['stockCode'])
                    stock_names.append(stock['stockName'])
                    stock_buy.append(stock['buyWorth'])
                if 'stockInfo' in get_data[i]:
                    del get_data[i]['stockInfo']
                for x in range(len(stock_names)):
                    entry = get_data[i].copy() 
                    entry['代码'] = stock_codes[x]
                    entry['股票'] = stock_names[x]
                    entry['净额(万元)'] = stock_buy[x]
                    new_data.append(entry)
            data = pd.DataFrame(new_data)
            get_title = ["capitalName","代码","股票","净额(万元)"]
            columns_chinese = { "capitalName": "游资"}
            data = data[get_title]
            data.rename(columns=columns_chinese, inplace=True)
            print("游资数据获取完成")
            # 获取脚本文件所在的目录
            script_dir = os.path.dirname(os.path.abspath(__file__))
            # 构建完整文件路径
            csv_filename = os.path.join(script_dir, "游资.csv")
...
def get_zrzt(zrzttoday):
    print("昨日获取涨停")
    stock_zt_pool_previous_em_df = ak.stock_zt_pool_previous_em(zrzttoday)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    # 构建完整文件路径
    csv_filename = os.path.join(script_dir, "昨日涨停.csv")
    stock_zt_pool_previous_em_df.to_csv(csv_filename, index=False, encoding='utf-8-sig')
    print("昨日涨停获取成功")




def start_get_data(today):
    print("开始获取数据")
    all_stocks()
    get_ths_hot_data()
    get_dfcf_hot()
    ...
    get_xueqiuhot_data()
    get_tgb_hot()
    zrzttoday = datetime.now().strftime('%Y%m%d')
    get_zrzt(zrzttoday)
    print("数据获取完成")
...
# # 添加一个标签
# label = tk.Label(root, text="点击按钮开始获取数据")
# label.pack(pady=10)
# today = datetime.now().strftime('%Y-%m-%d')
# # 添加一个按钮，设置其文本和绑定的函数
# btn_start = tk.Button(root, text="启动", command=lambda: start_get_data(today))
...
def create_chart():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    get_dfcf_hot_link = os.path.join(script_dir, "东方财富热搜.csv")
    get_dfcf_hot_link_df = pd.read_csv(get_dfcf_hot_link)
    st.dataframe(get_dfcf_hot_link_df)
create_chart()
